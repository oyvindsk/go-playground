...
func main() {
	...
	nk.NkTexteditInitDefault(&state.text2)
...
func gfxMain(win *glfw.Window, ctx *nk.Context, state *State) {
	...
	// Layout
	bounds := nk.NkRect(0, 0, winWidth, winHeight) // (50, 50, 230, 250)
	update := nk.NkBegin(ctx, "Demo", bounds,
		nk.WindowBorder|
			nk.WindowMovable|
			nk.WindowScalable|
			nk.WindowMinimizable|
			nk.WindowTitle|
			nk.WindowClosable,
	)
	...
	if update > 0 {
		...
		nk.NkLayoutRowDynamic(ctx, 30, 2)
		{
			nk.NkEditBuffer(ctx, nk.EditField, &state.text2, nk.NkFilterDefault)

		}

		nk.NkLayoutRowDynamic(ctx, 30, 2)
		if nk.NkButtonLabel(ctx, "Print Entered Text") > 0 {
			log.Println(state.text.GetGoString(), state.text2.GetGoString())
		}
